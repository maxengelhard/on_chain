...
class AevoLibClient:
    ...
    def get_markets(self, asset,instrument):
        req = self.client.get(f"{self.rest_url}/markets?asset={asset}&instrument_type={instrument}")
        data = req.json()
        return data
    ...
    def rest_create_market_order(self, instrument_id, is_buy, quantity,reduce_only):
        limit_price = 0
        if is_buy:
            limit_price = 2**256 - 1
    ...
    def rest_get_account_fundings(self):
        req = self.client.get(f"{self.rest_url}/account/accumulated-fundings", headers=self.rest_headers)
        return req.json()
    ...
    async def subscribe_tickers(self, asset,type):
        await self.send(
            json.dumps(
                {
                    "op": "subscribe",
                    "data": [f"ticker:{asset}:{type}"],
                }
            )
        )
    ...
    async def subscribe_markprice(self, asset,type):
        await self.send(
            json.dumps(
                {
                    "op": "subscribe",
                    "data": [f"markprice:{asset}:{type}"],
                }
            )
        )
